...
- A history pane behind the bars icon in the corner, or a click on any
  ...
  model's input and output tokens. Read from Claude Code's transcripts and
  Codex's session rollouts, sealed day by day into Notchlet's own archive,
  since Claude Code deletes transcripts after 30 days. Cost is what the tokens would cost at API list
  prices, never a bill.
