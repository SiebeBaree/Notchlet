...
/// The script pipes through `nc`, which only returns once this side
/// closes, so the read stops as soon as the message parses rather than
/// waiting for an EOF `nc` never sends. Each read runs on its own
/// background block, so a slow client never blocks the next one.
///
/// `nonisolated` because the accept and read closures run on dispatch
/// queues, where a main-actor closure traps.
final nonisolated class HookSocket {
    /// A dot folder: four CLIs quote the hook command four ways, and a path
    /// without spaces has nothing to quote. Also under the 104 byte limit
    /// on socket paths.
    ...
    /// Until the message parses, the peer closes, 300ms pass with nothing
    /// new, or the cap is hit.
