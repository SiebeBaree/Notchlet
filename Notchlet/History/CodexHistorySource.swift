...
/// under `archived_sessions/` once archived. Codex keeps them forever, so
/// here the archive only saves re-reading.
nonisolated struct CodexHistorySource: UsageHistorySource {
    ...
    /// A thread present under both roots keeps the live copy.
...
/// `token_count` carries the call's delta and the running total; the total
/// is the fallback for older rollouts and how a re-emitted snapshot (same
/// total twice) is told from a real call. Cached tokens are subtracted out
/// of Codex's input count.
///
...
/// creation. OpenUsage documents a 20x inflation from counting the replay.
nonisolated enum CodexLogParser: LogLineParser {
    struct State: Sendable {
        ...
        var isLive = true
        /// Epoch seconds, from `session_meta`.
    ...
    /// Large (it carries the system prompt) and shaped differently per
    /// version, so read as a dictionary rather than modeled.
